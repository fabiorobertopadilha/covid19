...
\qquad A pandemia da COVID-19 está impondo ao Brasil e ao mundo um desafio sem precedentes na história. No Brasil e no mundo, as autoridades implementam políticas públicas para conter o avança da doença e o crescente número de óbitos. Embora o Brasil, tenha o mais robusto sistema de saúde da America Latina, a capacidade de atendimento em todo país é bastante desigual.

\qquad Nas regiões mais pobre do Brasil onde a capacidade de atendimento médico é menor, verifica-se que a taxa de letalidade da doença é ainda maior.


\subsection{1.3 Objetivo}

\qquad É objetivo deste trabalho identificar as regiões com maiores número de óbitos, bem como as taxas de mortalidade e letalidade. Assim como, identificar os perfis dos pacientes com maiores riscos de morte. Para contribuir com as autoridades da área de saúde na construção de políticas públicas voltadas para redução dos números de óbitos a e contenção do novo corona vírus.
...
\qquad Dentre os dados utilizados neste trabalho, destacam-se os dados fornecidos pelo Ministério da Saúde por meio da Secretaria de Vigilância em Sáude no portal OpenDataSUS. Sobre os dados coletados informa-se no portal OpenDataSUS:
...
\qquad Como a Secretaria de Vigilância em Saúde atualiza o arquivo CSV semanalmente, para garantir o acesso ao arquivo exato, analisado neste traballho, fez-se necessário manter uma cópia do arquivo no Google drive. O arquivo pode ser acessado no link: \url{https://drive.google.com/drive/folders/1l52SO-RMHYfs2ci_2pBAZJNw2hA5i2me?usp=sharing}
...
\qquad Nessa seção também foram removidos do dataframe as notificações que não foram classificadas como casos de covid-19.
...
\qquad O processamento da coluna idade, foi atribuido a média da idade nacional, 59 (cinquenta e nove) anos nas ocorrências com valores superiores a 110 anos. Essa transformação teve como objetivo minimizar os outliner e os prováveis erros de digitação.
...
\qquad Na coluna CS_RACA, os valores numéricos foram substituidas pelas strings correspondentes, conforme descrito no dicionário de dados disponível na seção anexo.

\qquad O percentual de valores omissos na coluna CS_RACA é de 22% (vinte e dois por cento). 
...
\qquad Nesta seção constam os recursos que foram utilizados pelo paciente durante o tratamento. Os recurso são: se o paciente precisou de oxigêncio, se precisou de suporte de ventilação pulmonar, se foi pra UTI, se tomou algum antiviral ou se foi hospitalizado.

\qquad O percentual de valores omissões na coluna IN_HOSPITALIZADO é de 2.61% (dois virgula sesenta e um por cento).
...
\qquad O percentual de valores omissos nesta coluna é de 12.31%.
...
\qquad A taxa de letalidade foi calculada a partir semana da notificação.
...
\qquad Para a faixa etária, na primeira execução o modelo apresentou baixa significância nos intervalos de 5 a 9 anos e 10 a 14 anos, como também não exibiu a faixa de 0 a 4 anos. Assim, esses intervalos foram reagrupadas em um novo intervalo de 0 a 14 anos.
...
\qquad Nessa seção utiliza-se técnicas de machine learnig para identificar outros atributos com impacto no risco de óbitos dos pacientes, além dos atributos já descritos nas seções anteriores novos atributos serão incorporados aos modelos. Os algorítimos, também serão diversificados. 
...
\qquad Os dados utilizados nos treiamentos dos modelos, bem como os dados utilizados nos testes, serão extraídos a partir de uma amostra aletória com 1000 (um mil) registros. Sendo 70% (setenta por cento) da amostra destinada para o treinamento e 30%(trinta por cento) destinada para os testes.
...
FAIXA_ETARIA + CS_RACA + IN_FATOR_RISC + REGIAO , data = dadosMachineLearningObito, method = "class")
```
...
\qquad Com método randomForest e mantendo praticamente os mesmo atributos da execução anterior, com exceção do atributo REGIAO, substituido pelo atributo SG_UF a taxa de acerto referente ao risco de morte do paciente, apresenta uma pequena melhora. A após a mudança, a taxa de acertou passou para 70% (setenta por cento).
...
\qquad O gráfico abaixo apresenta os atributos mais significatívos na classificação de risco de morte do paciênte. A FAIXA_ETARIA do paciente é o atributo com maior peso na classificação do registro em relação ao risco de morte. Nas próximas execuções detalharemos mais os atributos FAIXA_ETARIA e SG_UF.
...
```{r arvore2}
arvoreClassificacao = rpart(formula =  CS_EVOLUCAO ~  NU_IDADE + MUNICIPIO, data = dadosMachineLearningObito, method = "class")
```
...
```{r arvore3}
arvoreClassificacao = rpart(formula =  CS_EVOLUCAO ~  NU_IDADE + MUNICIPIO + HOSPITAL, data = dadosMachineLearningObito, method = "class")
```
...
\qquad Com os mesmo atributos da seçao anterior e o médoto naiveBayes do pacote e1071 a taxa de acerto foi menor, 71% (setenta e um por cento)

```{r modeloNaiveBayes, results='hide'}
modeloNaiveBayes = naiveBayes(formula =  CS_EVOLUCAO ~  NU_IDADE + MUNICIPIO + HOSPITAL, data = dadosTreino)
modeloNaiveBayes
```
