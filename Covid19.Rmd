...
Após o ajuste do modelo e a reexecução em uma amostra com 436.361 (quatrocentos e trinta e seis mil e trezentos e sessenta e um) pacientes. Conclui-se que o perfil com maior risco de morte tem: idade de 80 (oitenta) anos ou mais ($\beta^1$ : 2.9126), já apresenta doenças preexistentes ($\beta^1$ : 0.5170), reside na região norte ($\beta^1$ : 0.3953), é de cor preta 
($\beta^1$ : 0.3310) e homem ($\beta^1$ : 0.2390), com probabilidade de óbito de 75% (setenta e cinco por cento).
...
Na primeira execução, o algorítimo utilizado será o rpart e os atributos serão os mesmos utilizados nas seção de inferência estatística. Com essa configuração a taxa de acerto do modelo foi 64% (sessenta e quatro por cento).
...
Com método randomForest e mantendo praticamente os mesmo atributos da execução anterior, com exceção do atributo REGIAO, substituído pelo atributo SG_UF a taxa de acerto referente ao risco de morte do paciente, apresenta uma pequena melhora. Depois da mudança, a taxa de acertou passou para 66% (sessenta e seis por cento).
...
Utilizando novamente a função rpart agora com atributo NU_IDADE e MUNICIPIO, a taxa de acerto passou a ser de 67% (sessenta e sete por cento)
...
taxaAcerto
```
Quando adicionado o hospital de atendimento do paciente como atributo do modelo, a taxa de acerto passa a ser de 70% (setenta por cento).
...
Com os mesmo atributos da seção anterior e o método naiveBayes do pacote e1071 a taxa de acerto foi menor, 68% (sessenta e oito por cento)
...
Nas seções acima fica evidenciado que no ano de 2020 as regiões mais pobres do país, Norte e Nordeste, foram as regiões que mais sofreram com a pandemia, com as maiores taxas de mortalidade e letalidade. A exceção do estado do Rio de Janeiro que apresentou a maior taxa de letalidade do país.

No que se refere ao perfil do pacienete foi verificado que a idade é o atributo que mais contribui para o aumento do risco de morte, seguido pela existência de comorbidades e sofrendo influência até do sexo. Os dados analisados demonstram que homens apresentam mais risco de óbito. 

A partir dos atributos selecionados e dos algoritmo de machine learning utilizados neste trabalho foi verificado uma taxa de acerto de até 70% (setenta por cento) dos casos graves de covid-19 que evoluiram no ano de 2020 para óbito.

\subsection{7.1 Dashboard-Covid19}

De maneira complementar, a este trabalho, foi elaborado no RStudio um dashboard na linguagem R, com auxílio das libs: flexdashboard, shiny, shinyWidgets e também das libs já descritas no primeiro capítulo. O deploy do dashboard foi realizado no shinyapps.io podendo ser acessado no link \url{https://fabiopadilhatccpucminas.shinyapps.io/Dashboard-Covid19/}.

O dashboard amplia a possíbilidade de consulta, indo além das informações apresentadas neste relatório. Ele permite também uma abordagem em drilldown sendo possível uma analise nacional, passando pelas regiões, estados chegando até o nível de municípios. A Figura \ref{fig:dashboardhome} mostra a página principal do dashboard, a figura \ref{fig:dashboardsobre} exibe a página de sobre com os créditos. 
...
O shinyapps.io é uma plataforma de serviço na núvem para o desenvolvimento de aplicações web usando R. A Figura \ref{fig:shinyappsio} exibe o deploy do dashboard no shinyapps.io.
...
file ="dashboard/dashboard_populacao.csv")
```
