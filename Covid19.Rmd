...
\qquad A planilha disponibilizada pelo IBGE contém cabeçalho e rodapé com metadados. Os dados populacionais necessários para este trabalho estão presentes da linha 3 até a linha 5572. Veja figura \underline{\color{blue}\ref{fig:arquivoIbge}} e a figura \underline{\color{blue}\ref{fig:rodapeIbge}} na página \underline{\color{blue}\pageref{fig:arquivoIbge}}.

\qquad A importação dos dados foi realizada a partir da linha três do arquivo, descartando o cabeçalho. Para descartar o rodapé, o data frame resultante da importação foi selecionado somente até a linha 5570. Por fim, foi criando um campo extra no data frame para armazenar o código do município sem o último dígito.

\qquad O campo criado com o código do município sem o dígito verificador, será utilizado em JOINs nas seções seguintes.
...
\qquad No que se refere à raça do paciente, o modelo apresentou baixa significância para raça branca e não exibiu a raça amarela. Então, optou-se por agrupar as raças amarela e branca em um grupo misto. Conforme tabela \underline{\color{blue}\ref{tab:regTabLog1}}
